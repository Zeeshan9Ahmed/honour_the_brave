...
var con_mysql = mysql.createPool({
    host: "localhost",
    user: "server1appsstagi_honour_the_brave_user",
    password: "zbE#FA9nY{W?",
    database: "server1appsstagi_honour_the_brave",
    debug: true,
    charset:'utf8mb4'
});

var FCM = require('fcm-node');
var serverKey = 'AAAAZX0EosQ:APA91bHDQAcy68t1zBKz7hLT3LkSf15Su_xkudVooIjIRUdE9QiNcRKi260NGIdFzgKW8Eu_Uzma1YeXHVaQ4oljvW2Yoz0ZU4gi22QwNv8IGbNb5WwP4gkxhCzfjIOVXGykdmilGGK7'; //put your server key here
...
var get_messages = function(object, callback) {
    con_mysql.getConnection(function(error, connection) {
        if (error) {
            ...
            order by chats.chat_id ASC`, function(error, data) {
                connection.release();
                if (error) {
                    callback(false);
                } else {
...
var group_get_messages = function(object, callback) {
    con_mysql.getConnection(function(error, connection) {
        if (error) {
            ...
            chats.created_at
            from chats 
            inner join st_user on chats.chat_sender_id = st_user.user_id
            WHERE chats.chat_group_id=${object.group_id} order by chats.chat_id ASC`, function(error, data) {
                connection.release();
                if (error) {
                    callback(false);
                } else {
...
var send_message = function(object, callback) {
    ...
    con_mysql.getConnection(function(error, connection) {
        if (error) {
            ...
            connection.query(`INSERT INTO chats (chat_sender_id , chat_reciever_id , chat_message, chat_type,created_at,updated_at) VALUES ('${object.sender_id}' , '${object.reciever_id}', '${new_message}', '${object.chat_type}',NOW(),NOW())`, function(error, data) {
                if (error) {
...
var group_send_message = function(object, callback) {
    ...
    con_mysql.getConnection(function(error, connection) {
        if (error) {
            ...
            var new_message = mysql_real_escape_string (object.message);
            connection.query(`  INSERT INTO chats (chat_sender_id , chat_group_id , chat_message,created_at) VALUES ('${object.sender_id}' , '${object.group_id}', '${new_message}',NOW())`, function(error, data) {
                if (error) {
                    console.log("FAILED TO VERIFY LIST")
                    callback(false);
                } else {
...
var delete_message = function(object, callback) {
    con_mysql.getConnection(function(error, connection) {
        if (error) {
            ...
            connection.query(`delete from chats where chat_id = '${object.chat_id}'`, function(error, data) {
                if (error) {
                    console.log("FAILED TO VERIFY LIST")
                    callback(false);
                } else {
...
// SERVER LISTENER
server.listen(3076, function() {
    console.log("Server is running on port 3076");
});